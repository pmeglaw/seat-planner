...
- **34 migration files; 10 are deliberate no-op `select 1;` placeholders** reconciling the Supabase migration ledger with what production already recorded (two events: the legacy `00N_*` → timestamped rename, and PR #32 where the connector applied timestamps before the code merged). Legacy `00N_*` files sort before all `2026*` files, so apply order is correct.
  - Most placeholders name their real migration in the filename or carry an "Equivalent source migration:" comment. **Exactly two are opaque** — bare `select 1;` under a generic `_placeholder` name — and their mappings are recorded here because the files themselves are already applied and should not be edited to add comments:
    - `20260506235849_placeholder.sql` → ledger `20260506235849 departments_zones_management` ≡ `007_departments_zones_management.sql`
    - `20260520235335_placeholder.sql` → ledger `20260520235335 publish_audit_logging_hardening` ≡ `20260521000100_publish_audit_logging_hardening.sql`
  - (Rescued 2026-07-22 from `docs/supabase-migration-ledger-reconciliation-plan.md` before that doc was retired.)
