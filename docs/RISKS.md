...
| R-02 | `SeatMap.tsx` god component (2,684 lines, ~85 hooks) + unmemoized marker layer | High | Large (incremental) | **(a) Open, improving** — 3,513 lines (audit baseline 2,684; had grown to 3,966 before the M4 extractions began) after M4 step 1 (all seven confirm/review dialogs → `SeatMapDialogs.tsx`) and step 2 (publish-review state, fence capture, diff memos and both confirm handlers → `usePublishReview.ts`), both 2026-08-12; earlier `useDraftHistory`/`useSeatDraftActions`/`useInspectorNudge`/`MapWashLayer` extractions also landed. Next per M4: filters/search state → `useSeatFilters`. **(b) Resolved** — `SeatMarker` memoized with a custom comparator (`seat-marker-memo` test); render loop fixed (#354) |
