...
| R-02 | `SeatMap.tsx` god component (2,684 lines, ~85 hooks) + unmemoized marker layer | High | Large (incremental) | **(a) Open, improving** — 3,406 lines (audit baseline 2,684; had grown to 3,966 before the M4 extractions began) after M4 step 1 (all seven confirm/review dialogs → `SeatMapDialogs.tsx`), step 2 (publish-review state, fence capture, diff memos and both confirm handlers → `usePublishReview.ts`), and the filters/search step (filter values, derived results and handlers → `useSeatFilters.ts`), all 2026-08-12; earlier `useDraftHistory`/`useSeatDraftActions`/`useInspectorNudge`/`MapWashLayer` extractions also landed. The M4 list is complete; what remains in SeatMap is selection/mode state, viewport/pan/zoom, and the JSX shell — further slicing is possible but past the planned decomposition. **(b) Resolved** — `SeatMarker` memoized with a custom comparator (`seat-marker-memo` test); render loop fixed (#354) |
...
| R-16 | Assorted UI-tier duplication and hygiene (two Buttons, duplicated filter logic, inline dialogs, hex bypasses, UUID leak in publish review) | Low | Medium (bundled) | **Partially resolved** — one Button; viewer shares `FilterPanel`; wash fills tokenized (#367); SeatMap's inline dialogs extracted to `SeatMapDialogs.tsx` (2026-08-12; the per-dialog overlay/aria scaffolding is still duplicated within that file). Still open: the `Employee ${id}` UUID fallback in `lib/publishSummary.ts` |
