...
| R-02 | `SeatMap.tsx` god component (2,684 lines, ~85 hooks) + unmemoized marker layer | High | Large (incremental) | **(a) Open, improving** — 3,583 lines after the M4 step-1 dialog extraction (all seven confirm/review dialogs → `SeatMapDialogs.tsx`, 2026-08-12); earlier `useDraftHistory`/`useSeatDraftActions`/`useInspectorNudge`/`MapWashLayer` extractions also landed. Next per M4: publish-review state → hook, then filters/search. **(b) Resolved** — `SeatMarker` memoized with a custom comparator (`seat-marker-memo` test); render loop fixed (#354) |
