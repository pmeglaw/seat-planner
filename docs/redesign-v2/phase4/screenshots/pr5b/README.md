...
| `npm run test:e2e:auth` | **53 passed** (2.6 min) on a reset + reseeded stack (its `webServer` rebuilt the head with the local URL + anon key inline): publish flow, nav-shell, header-geometry, page-frames, accessibility, **draft-dialogs** (all seven dialogs opened + axe, the Delete X99 for real, every cancel path through the dialog's Cancel), reception-keyboard |
