...
**Status: in progress — PR 0 #512 merged (v1.74.0). PR 1 #513 merged (v1.74.1). 1b #514 merged (v1.74.2). PR 2 #515 merged (v1.74.3). PR 3a #516 merged (v1.74.5 — v1.74.4 went to chore #517). PR 3b #518 merged (v1.74.6, 2026-09-05). PR 4 #519 merged (v1.75.0, 2026-09-05). PR 5 #522 merged (v1.76.0, 2026-09-07). PR 5b #523 merged (v1.77.0, 2026-09-07). **PR 6 — the close-out — MERGED 2026-09-08: #524 squashed as `0caedc6`, annotated tag **v2.0.0** "Phase 4 — redesign complete", production READY at the merge SHA (`dpl_6CG94vo3Bo32uUJqLeXRYWTMt4X9`, seats.megeredchianlaw.com), migration `20260908120000_deactivate_employee_sqlstate` applied, all four stale branches pruned. Phase 4 is COMPLETE.** Inputs, in reading order:
...
**The moved F-8 obligation is DISCHARGED — the read-only walk on production, 14 / 14 (2026-09-08).**
`audit/pr6-preview-walk.mjs` against `https://seats.megeredchianlaw.com` on the merge SHA
(`/api/build-id` = `0caedc6e…`), the owner signing in by hand, 1920 × 1080 in **both themes**, on the live data
(68 seats). Per theme: with the slot closed the band's Zoom-in sits at (1896, 1060), `padding-right: 8px`, and
hit-tests to itself; with the inspector open it sits at **(1496, 1060)**, `padding-right: 408px`, **8px clear** of the
slot, and still hit-tests to the button (`inBand: true`, `inSlot: false`) — the count reads "68 seats", unclipped.
Pressed for real: zoom in took the plan 1504 → **1911**, Fit returned it to 1504, with **zero** new action POSTs.
**Zero writes proven:** the indicator read "Draft — no changes" and Undo "No map changes to undo" before the walk and
after every step, no success notice ever appeared, and the whole session sent **one** server-action POST — the
shell's argument-less status read on entry (200). Captures and provenance: `screenshots/pr6-prod-walk/`.
