...
| 4 | Mode indicator | centred in the header's free run between the last section link and the first utility (*amended 2026-09-04, owner ruling in Phase 4 PR 2 — was "centred on 960", which met the admin's links between 1056 and ~1580px; at 1920 the admin's indicator centres at x = 1227, the viewer's at 1127 — measured by `tests/e2e-auth/header-geometry.spec.ts`*) | Status-only button. **Published:** filled square 12px + "Published · 2 Sep 2026". **Draft:** hollow diamond 12px (2px stroke) + "Draft — 4 changes". Press opens the History panel: `aria-expanded` + `aria-controls`, non-modal | D0-a. Two signals in the mark (shape + fill), text third. Date format = the app's existing publish-date formatter. Never collides: links end ≈ 688, utilities start at 1776 |
...
| Narrow (1024) | Single column, and the readout **splits by job** — *Phase 5 PR 2 amendment (2026-09-08, owner ruling R1; DECISIONS **D3-f**; sheet amendment I), replacing "locking scrolls to the readout below with a **Back to the list** ghost link at its top".* Order: page header, search, then the **band** — name · extension · seat line — pinned under the search (`position: sticky`) so the list scrolls beneath it; then the list; then the **tail** — the same-department fallbacks and Show on map, in §1R.4's shipped order — and Recent lookups. **Why it changed:** amendment E's single stacked readout was free while the row extension was 20px semibold and the list answered by itself; at `--sp-type-code-02` it does not, so the one number read aloud sat below the fold and every lookup was a scroll down and back up. Holds **480 → 1055** (R2 — she drags the window, so it is proved across the range, not at one width); below 420 the name block wraps under the numeral, which is what keeps §2's 320 reflow floor working. **Back to the list is retired** (D3-f): the list is never left, so there is nothing to go back from — and that is what leaves the band with no focusable element, so focus order still matches visual order (WCAG 2.4.3; C27 is a sufficient technique, not a criterion — reviewer ruling O-1) |
