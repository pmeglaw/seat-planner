...
- **BR-3 withdrawn.** The finding placed the in-row "Add to list" tertiary on a layer-01 `.cds-table tbody tr` (`#f4f4f4`, 4.14:1) and its hovered row (`#e8e8e8`, 3.72:1). `OptionList.tsx` renders no table: the button sits in `<li class="sp-list-row">` inside `<ul class="sp-list">`, and `.sp-list-row` (`sp-components.css:960`) paints no fill — `display: grid … box-shadow: inset 0 -1px 0 var(--sp-list-rule)`, no `background` — and has no `:hover` rule (`--sp-list-hover`, `sp-tokens.css:488`, has no consumer). The ground under the button is `--sp-background` (`#ffffff` light — the route pane at `app/(shell)/admin/management/page.tsx:99` and `body` at `globals.css:77`), where tertiary text `#B85C2E` measures **4.56:1** (pass, 4.5 floor); the button's only hover state is its own `#8F4521` fill with white text, **6.91:1** (`carbon-components.css:142`). The fix-pack ghost (`a4ee33b`, recorded in `2331946`) is reverted to the tertiary PHASE3DS §1.25 specifies and the §1.25 amendment is removed. Dark is unaffected: the brand file overrides `--cds-button-tertiary` only in the light block, and g100's tertiary is `#ffffff`. The §4.3 table rows stand as written — this record is frozen; the erratum is the correction.
