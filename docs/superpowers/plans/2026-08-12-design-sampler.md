# Design Sampler (Phase 0) Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ship `app/concepts/design-sampler/` — a gated prototype page rendering the same six-component set in three visual archetypes (Ethereal Glass, Editorial Luxury, Soft Structuralism) so the owner can pick the winning language in a real browser.

**Architecture:** One theme-parameterized client component: three archetype config objects (class strings + copy) fed through a single `ArchetypeSection` renderer, so the comparison is apples-to-apples by construction. Fonts vendored under a shared `app/concepts/fonts/`. A new source test pins the `prototypesEnabled()` gate on every concepts page.

**Tech Stack:** Next.js App Router (client component below a server-gated `page.tsx`), Tailwind arbitrary values, `next/font/local`, `node --test` source test.

**Spec:** `docs/superpowers/specs/2026-08-12-highend-visual-redesign-concepts-design.md` — this plan implements Phase 0 only. Phases 1–3 (viewer-v13 / admin-v13 / login-v13) are planned separately AFTER the owner's sampler ruling, because the winning archetype determines their entire visual vocabulary.

## Global Constraints

- Prototype gate verbatim on every concepts page: `process.env.NODE_ENV !== "production" || process.env.SEAT_PLANNER_ENABLE_PROTOTYPES === "true"`, else `notFound()`; metadata `robots: { index: false, follow: false }`.
- Zero imports from `lib/supabase/*`, zero server actions, zero live-data reads. Importing display constants from `lib/mapLayoutTransform.ts` is allowed (map-redesign precedent).
- No `next/font/google` anywhere. Fonts are vendored files loaded via `next/font/local` **with an explicit `weight` range** (omitting it silently degrades every weight to synthetic-bold-over-400 — see `app/concepts/component-state-board/ComponentStateBoard.tsx:30-39`).
- No changes to `app/globals.css`, `tailwind.config.ts`, `app/actions.ts`, `proxy.ts`, `AppShell`, or any shipped surface. `package.json`/`package-lock.json` must be byte-identical at the end (font packages are installed only to source files, then uninstalled).
- All entry/reveal animations guarded by `prefers-reduced-motion`. Animate only `transform`/`opacity`. No `backdrop-blur` on elements that scroll with the page (skill's glass-card blur is deliberately dropped; an in-page footnote says so, per spec §7).
- Windows-local/Linux-CI path rule: in the new test, derive paths with `fileURLToPath`, never `new URL(...).pathname`.
- Repo completion bar: `npm test`, `npm run typecheck`, `npm run lint`, `npm run build` all green; visual verification is a separate mandatory step via the `run-seat-planner` skill.

---

### Task 1: Vendor sampler fonts under `app/concepts/fonts/`

**Files:**
- Create: `app/concepts/fonts/geist-latin-wght-normal.woff2`
- Create: `app/concepts/fonts/fraunces-latin-wght-normal.woff2`
- Create: `app/concepts/fonts/plus-jakarta-sans-latin-wght-normal.woff2`
- Create: `app/concepts/fonts/LICENSE-geist`, `LICENSE-fraunces`, `LICENSE-plus-jakarta-sans`
- Create: `app/concepts/fonts/README.md`

**Interfaces:**
- Produces: the three woff2 files above, referenced by Task 4's `localFont` calls as `../fonts/<name>.woff2` with weight ranges Geist `"100 900"`, Fraunces `"100 900"`, Plus Jakarta Sans `"200 800"`.

- [ ] **Step 1: Source the files from fontsource (install → copy → uninstall)**

```bash
npm i -D @fontsource-variable/geist @fontsource-variable/fraunces @fontsource-variable/plus-jakarta-sans
mkdir -p app/concepts/fonts
...
npm view @fontsource-variable/geist version && npm view @fontsource-variable/fraunces version && npm view @fontsource-variable/plus-jakarta-sans version
npm un @fontsource-variable/geist @fontsource-variable/fraunces @fontsource-variable/plus-jakarta-sans
```

Record the three printed versions — they go in the README table. Fallback if `@fontsource-variable/geist` does not exist: `npm i -D geist` (Vercel's official package, OFL) and copy `node_modules/geist/dist/fonts/geist-sans/Geist-Variable.woff2` (rename to `geist-latin-wght-normal.woff2`) plus its `LICENSE.txt`; note the substitution in the README table.

- [ ] **Step 2: Verify repo state**

Run: `git status --short`
Expected: ONLY new files under `app/concepts/fonts/`. `package.json` / `package-lock.json` must NOT appear (if they do, the uninstall didn't restore them — fix before committing).

- [ ] **Step 3: Write `app/concepts/fonts/README.md`**

Follow the `app/fonts/README.md` shape (provenance table, pinned refresh recipe, per-family license note). Content:
...
| --- | --- | --- | --- |
| `geist-latin-wght-normal.woff2` | `@fontsource-variable/geist` | <recorded> | 100–900 |
| `fraunces-latin-wght-normal.woff2` | `@fontsource-variable/fraunces` | <recorded> | 100–900 |
| `plus-jakarta-sans-latin-wght-normal.woff2` | `@fontsource-variable/plus-jakarta-sans` | <recorded> | 200–800 |
...
```bash
npm i -D @fontsource-variable/geist@<ver> @fontsource-variable/fraunces@<ver> @fontsource-variable/plus-jakarta-sans@<ver>
cp node_modules/@fontsource-variable/geist/files/geist-latin-wght-normal.woff2 app/concepts/fonts/
cp node_modules/@fontsource-variable/fraunces/files/fraunces-latin-wght-normal.woff2 app/concepts/fonts/
cp node_modules/@fontsource-variable/plus-jakarta-sans/files/plus-jakarta-sans-latin-wght-normal.woff2 app/concepts/fonts/
cp node_modules/@fontsource-variable/geist/LICENSE app/concepts/fonts/LICENSE-geist
...
Replace `<recorded>` / `<ver>` with the versions from Step 1 before committing.

- [ ] **Step 4: Commit**

```bash
git add app/concepts/fonts/
git commit -m "feat(concepts): vendor Geist, Fraunces, Plus Jakarta Sans for prototype pages"
```

---

### Task 2: Gate source test for all concepts pages

**Files:**
- Create: `tests/concept-gate-source.test.mjs`

**Interfaces:**
- Produces: a test that auto-discovers every directory under `app/concepts/` containing a `page.tsx` — Task 3's new page is covered with no test edit. Directories without a `page.tsx` (e.g. `fonts/`) are skipped.
...
- [ ] **Step 2: Run it — must pass on the three existing pages**

Run: `node --test tests/concept-gate-source.test.mjs`
Expected: PASS, 4 tests (1 count + 3 pages). If any existing page fails, the regexes drifted from the real gate pattern — fix the test, not the pages.

- [ ] **Step 3: Commit**

```bash
git add tests/concept-gate-source.test.mjs
git commit -m "test: pin prototypesEnabled gate on every app/concepts page"
```

---

### Task 3: Sampler route scaffold (gate proven by failing test first)

**Files:**
- Create: `app/concepts/design-sampler/DesignSampler.tsx` (minimal shell, replaced in Task 4)
- Create: `app/concepts/design-sampler/page.tsx`

**Interfaces:**
- Consumes: Task 2's auto-discovery (a `page.tsx` under a new concepts dir is tested automatically).
- Produces: route `/concepts/design-sampler`; `DesignSampler` named export that Task 4 fills in.
...
- [ ] **Step 2: Create a deliberately gate-less page and watch the test fail**

Create `app/concepts/design-sampler/page.tsx` containing only:
...
Run: `node --test tests/concept-gate-source.test.mjs`
Expected: FAIL — `prototype gate intact: design-sampler/page.tsx` reports "prototypesEnabled() condition missing or altered". This proves the test actually guards new pages.

- [ ] **Step 3: Replace with the real gated page**

```tsx
// app/concepts/design-sampler/page.tsx
...
Run: `node --test tests/concept-gate-source.test.mjs`
Expected: PASS, 5 tests (1 count + 4 pages).

- [ ] **Step 5: Commit**

```bash
git add app/concepts/design-sampler/
git commit -m "feat(concepts): scaffold gated design-sampler route"
```

---

### Task 4: Sampler content — three archetypes over one component set

**Files:**
- Modify: `app/concepts/design-sampler/DesignSampler.tsx` (replace shell entirely)

**Interfaces:**
- Consumes: Task 1 font files (`../fonts/*.woff2`); `MAP_IMAGE_SRC`, `MAP_IMAGE_BLUR_DATA_URL` from `lib/mapLayoutTransform.ts` (map-redesign precedent).
- Produces: the reviewable page. CSS vars `--font-sampler-grotesk`, `--font-sampler-serif`, `--font-sampler-soft`.

The component is theme-parameterized: three `ArchetypeTheme` objects hold ALL styling differences; one `ArchetypeSection` renders the identical component set from each. The code below is the complete structure and the complete theme objects — the executor types it in, extends copy only where marked, and does not invent new components.
...
- The `!p-0` on the map core is deliberate: the map bleeds to the core's radius; pills sit inside the crop.
- Copy may be polished, structure may not: exactly three themes, exactly this component set, styling only via the theme objects.
- If `@/lib/mapLayoutTransform` alias fails, use the relative import `../../../lib/mapLayoutTransform` (check how `MapRedesignPreview.tsx` imports it and match).

- [ ] **Step 2: Static gates**

Run: `npm run typecheck && npm run lint`
Expected: both clean. Common trips: `ReactNode`/`CSSProperties` must be type-only imports; `aria-hidden` boolean form.

- [ ] **Step 3: Full test suite**

Run: `npm test`
Expected: green, including `concept-gate-source` (5 tests) and all existing source tests. If `accessibility-source` flags the page, fix the page (focus rings and labels are already in the code above — a trip means a real omission).

- [ ] **Step 4: Look at it (mandatory, not optional)**

Use the `run-seat-planner` skill: start dev (`SEAT_PLANNER_ENABLE_PROTOTYPES=true` in env), open `/concepts/design-sampler`, verify all three sections render, fonts differ per section (serif visible in Editorial Luxury), reveals fire on scroll, hover physics on all three CTAs, focus ring visible on inputs and anchor pills via keyboard Tab. Screenshot each section for the PR body.

- [ ] **Step 5: Commit**

```bash
git add app/concepts/design-sampler/DesignSampler.tsx
git commit -m "feat(concepts): design-sampler — three archetypes over one component set"
```

---

### Task 5: Docs, build proof, PR

**Files:**
- Modify: `CLAUDE.md` (concepts list, one line)
- Verify only: full build with the flag unset

**Interfaces:**
- Consumes: everything above.
- Produces: PR for owner review; spec changelog stays "Sampler ruling: pending" until the owner drives it.

- [ ] **Step 1: Update the concepts list in CLAUDE.md**

In the `app/concepts/` paragraph, change the page enumeration to include the new page:

`component-state-board`, `map-redesign`, `login-v12` → `component-state-board`, `map-redesign`, `login-v12`, and `design-sampler`

No other CLAUDE.md changes.

- [ ] **Step 2: Prove the production gate**

Run: `npm run build`
Expected: build succeeds with `SEAT_PLANNER_ENABLE_PROTOTYPES` unset; `/concepts/design-sampler` prerenders as 404 (same as the three existing concepts — check the build output route table).
...
```bash
git add CLAUDE.md
git commit -m "docs: add design-sampler to the concepts list"
git push -u origin HEAD
gh pr create --title "feat(concepts): design-sampler — archetype pick for the high-end visual redesign" --body "$(cat <<'EOF'
## Summary
- Phase 0 of docs/superpowers/specs/2026-08-12-highend-visual-redesign-concepts-design.md
- New gated prototype /concepts/design-sampler: three archetypes (Ethereal Glass, Editorial Luxury, Soft Structuralism) rendering the SAME six-component set, theme-parameterized
- Vendors Geist / Fraunces / Plus Jakarta Sans (OFL) under app/concepts/fonts/ with README + licenses
- New tests/concept-gate-source.test.mjs pins the prototypesEnabled() gate on every concepts page (auto-discovers new ones)

## Review ask
Drive it live (SEAT_PLANNER_ENABLE_PROTOTYPES=true npm run dev → /concepts/design-sampler) and pick the winning archetype — that ruling gates Phases 1–3 (viewer/admin/login prototypes).
...
1. **Spec coverage:** §2 constraints → Global Constraints + Task 3 gate + Task 4 reduced-motion/blur handling; §3 Phase 0 component set → Task 4 (all six present); §5 testing → Task 2 + Task 5; §6 process → branch exists, Task 5 PR. Phases 1–3 deliberately out (spec's own ruling gate) — noted in header.
2. **Placeholders:** README `<recorded>`/`<ver>` are executor-filled from Step 1 output with explicit instruction — acceptable; no TBD/TODO remain.
3. **Type consistency:** `ArchetypeTheme` fields match all three theme objects and every use in `ArchetypeSection`; `useReveal`/`Reveal` names consistent; CSS vars match `localFont` declarations.
