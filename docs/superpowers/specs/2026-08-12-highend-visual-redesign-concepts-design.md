...
- 2026-08-12 — Phases 2–3 delivered: `app/concepts/admin-v13/` and `app/concepts/login-v13/` built in the ratified glass language (#FF7A1F). admin-v13's editor shell (rail, map, inspector, publish-review dialog, unsaved-edits pill) and login-v13's two-step progressive login (flow rulings frozen per §2.5; skin only) are both live behind the prototype gate. Owner rulings per surface remain pending — see below.
