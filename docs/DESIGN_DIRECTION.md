...
| Validation screenshots | `docs/design-review/ember/` — desktop admin (default, inspector, search, filters, demo states, names off, marker focus), viewer (desktop + inspector), mobile (admin, sheet, viewer), 2× marker close-ups on the real floor, plus the `study-full-*.png` full-occupancy set (orange names/initials vs neutral plates) that drove the assigned-plate decision |
...
3. **One orange, kept scarce.** `#F45B2A` is THE brand orange — Publish, the selection glow, assigned-status dots, brand moments — with a single ramp: `#FF7138` hover, `#D94A1F` pressed, `#A93818` deep (borders/halos on light contexts). **Ink `#140D04` is the locked text color on every orange fill** (5.85:1 rest / 7.04 hover / 4.54 pressed; white fails at 3.29). Orange is never used for hierarchy, and it is **not a plate fill**: the full-occupancy study showed orange plates become wallpaper at 60/60, so occupied plates are neutral dark and orange stays a signal (owner call 2026-07-09).
...
- **Open seat:** 16px **white dot with 2px `#3E4650` slate ring** — the white casing + dark ring pair is what clears 3:1 on every measured floor patch (§5); seat code appears as a tag on hover/focus (dark `#22262D` chip, light text). **Custom seat:** dashed ring (a distinct visual state). **Occupied (assigned):** name plate in **neutral dark `#22262D` with `#48515E` border, white `#F8FAFC` name, muted `#A7ADB5` code, and an orange `#F45B2A` status dot** — the plate reads like a gallery label on the lit floor. This is the owner-selected outcome of the 60/60 full-occupancy study (the Ember spec's orange plates drown the map at occupancy); it is also the strongest mark measured — the dark fill alone clears **every** sampled patch (min 4.02:1, typ. ≥7.2:1), name 14.51:1, dot 4.61:1 vs fill. Names-off mode collapses plates to initials chips (same anatomy).
- **Selected:** **ember glow `rgba(244,91,42,.35)` + a counter-ring** + elevation + 1.06 scale (motion-safe), raised above neighbouring plates (plates collide at real density — the selected one always wins z-order; verified live at W08/W09). The counter-ring takes the opposite layer of whatever it wraps: **white with a fine ink edge on the dark assigned plate** (white vs plate 15.18:1; ink edge vs floor ≥9.11:1), **plain ink `#140D04` on light marks** — dots (19.28:1 vs fill, ≥5.11:1 vs floor), reserved plates (16.78:1), match plates (15.93:1). The spec's `#FF7138` ring measured 1.00:1 on the worst floor patch — rejected; the glow keeps the moment orange. **Search/filter match:** teal `#DCEDEA` tint + `#1D4042` ring/text; non-matches dim 40%. **Reserved:** cream `#FBEED3` plate with `#7A4E00` border/dot/tagword (spec's `#F59E0B` measured 1.01:1 on the floor — rejected). **Unavailable:** white dot + `#6B7280` muted ring + **ink strike glyph** (the strike doubles as the dark contrast layer). **Draft-changed (admin):** corner tick in the orange dot family (shape + position channel; final geometry in Phase 4). Admin mode states carry the 13-state taxonomy forward on the same two-layer rule — target-valid pairs a light tint with floor-safe green `#166534`, target-invalid a neutral plate with brick `#963D2F` accent, move-origin a dashed ring.
...
| *(brand deep — new)* | — | **`#A93818`** | deep ramp step — halos/accents on light contexts (3.04:1 worst-case vs floor) |
...
| *(marker plate fill — new)* | *(white plates)* | **`#22262D`** | neutral dark plates (owner call after the 60/60 study); fill alone clears every patch, min 4.02:1 |
| `--sp-marker-plate-border` | *(planned `#C9C3BA`)* | **`#48515E`** | decorative (1.89:1 vs fill); the boundary is carried by the dark fill itself |
| `--sp-marker-plate-text` / `-code` | *(planned `#201D1A`/`#5E574E`)* | **`#F8FAFC` / `#A7ADB5`** | 14.51:1 / 6.71:1 on the fill |
| `--sp-marker-assigned` (status dot) | *(planned `#3F6F59`)* | **`#F45B2A`** | THE orange as a scarce signal; 4.61:1 vs the plate fill; green leaves the marker family |
...
| `--sp-marker-selected-ring` | *(planned `#B8541A`)* | **counter-ring + `rgba(244,91,42,.35)` glow** — white w/ ink edge on dark plates; `#140D04` on light marks | white vs plate 15.18:1, ink edge vs floor ≥9.11:1; ink ring ≥5.11:1 vs floor, 15.93–19.28:1 vs light fills |
...
**Call #2 — markers on the real floor (resolved; worst-seat numbers).** Open dot (white + `#3E4650` ring): FULL coverage, best-layer min **3.18** (@CW06), ring vs own fill **9.56**. Match dot/plate (`#DCEDEA` + `#1D4042`): FULL, min **3.12** (@NE04); match text on tint **9.30**. Assigned plate (`#22262D`, `#48515E` border, `#F45B2A` dot — owner-selected after the full-occupancy study): the dark fill alone clears every sampled patch — min **7.17** across plate footprints (@N01), **4.02** even against the harshest dot-footprint patch (@NE04); name `#F8FAFC` **14.51**, code `#A7ADB5` **6.71**, orange dot **4.61** vs fill. Selected (counter-ring + ember glow): on the dark plate, white ring vs plate **15.18** with ink outer edge vs floor min **9.11** (@N01); on light marks, ink `#140D04` ring — min **5.11** vs floor (@NE04), vs white dot **19.28**, vs reserved tint **16.78**, vs match tint **15.93**; glow decorative. Marker focus (white + `#542D12`): FULL, min **3.52**; ring pair internal **11.92**. Unavailable (white dot + ink strike): FULL; strike vs fill **15.26**; `#6B7280` ring is the hue cue. Reserved plate (`#FBEED3` + `#7A4E00`): min **3.40** (@N01); text on tint **6.26**. Department floor-safe accents: mins **3.04–3.43** (@N01).
