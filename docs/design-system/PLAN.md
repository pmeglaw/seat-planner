...
- **Redesign input list** (items the `docs/redesign` decision document inherits, beyond R1–R5): the rail-over-employee-dialog overlap at 390px (#476 live pass, recorded not fixed — handoff §5); and, from the 2026-08-29 hardware-target correction (desktop dual 27" FHD, addendum in `READ-PATH-ASSESSMENT.md`), the reopened read-path rulings — **F1** (hover-disclosed name held at 10px), **F2** (hover occluding the neighbour's code), **F3** (first-name-only cue; marker `title` removed), **F5** (no hover path on touch) — reopened as inputs, not reverted: on the real hardware scanning the map is a legitimate first path and the palette is a shortcut, so whether hover becomes a real disclosure again (and whether `title` returns) is the redesign's to decide.
