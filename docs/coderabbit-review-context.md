...
4. **Security boundary — two enforcing layers, plus session refresh.** (a) every exported server action calls `requireAdmin()`; (b) Postgres RLS + SECURITY DEFINER RPCs enforce admin independently. The root `proxy.ts` refreshes the session cookie and fails open — it enforces nothing. Client guards (`adminPageGuard`) are UX only. Service-role key and `OPENAI_API_KEY` never reach the browser.
