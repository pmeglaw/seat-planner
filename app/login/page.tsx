...
export default async function LoginPage() {
  ...
  return (
    ...
    <main className="login-theme flex min-h-screen flex-col bg-[var(--sp-background)] lg:flex-row lg:items-stretch">
      <section className="sp-zone-chrome relative flex flex-col overflow-hidden bg-[var(--sp-background)] p-8 lg:min-w-0 lg:flex-[1.2] lg:p-12">
        <div className="flex items-center gap-3">
          ...
          />
          <span translate="no" className="text-[14px] font-semibold leading-none text-[var(--sp-text-primary)]">
            Megeredchian Law
          </span>
        </div>
        ...
        <div className="mt-8 max-w-[520px] lg:mt-0">
          <span aria-hidden="true" className="mb-6 block h-[3px] w-12 bg-[var(--sp-button-primary)]" />
          <h1 className="text-[42px] font-light leading-[1.2] text-[var(--sp-text-primary)]">Seat Planner</h1>
          <p className="mt-3.5 hidden text-[15px] leading-[1.55] text-[var(--sp-text-secondary)] lg:block">
      ...
      <section className="grid flex-1 place-items-center bg-[var(--sp-background)] px-6 py-10 sm:px-10">
        ...
        <div className="w-full max-w-[368px] motion-safe:animate-[login-rise-in_0.5s_cubic-bezier(0,0,0.38,0.9)_both]">
          {user?.email ? (
            <div className="flex flex-col">
              <h2 className="text-[28px] font-normal leading-[1.25] text-[var(--sp-text-primary)]">
                Already signed in
              </h2>
              <p className="mt-2 text-[12.5px] leading-[1.5] text-[var(--sp-text-secondary)]">
                You’re signed in as{" "}
                <span className="font-semibold text-[var(--sp-text-primary)]">{user.email}</span>.
              </p>
              <Link
                href="/"
                className="mt-6 flex h-12 w-full items-center justify-between gap-3 bg-[var(--sp-button-primary)] px-4 text-[13.5px] font-medium leading-none text-white hover:bg-[var(--sp-button-primary-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[color:var(--sp-focus)] focus-visible:ring-offset-2"
              >
              ...
              <form action="/auth/signout" method="post" className="mt-3">
                <button
                  type="submit"
                  className="flex h-12 w-full items-center border border-[var(--sp-border-strong)] bg-transparent px-4 text-[13px] leading-none text-[var(--sp-text-primary)] transition-colors hover:bg-[var(--sp-field-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[color:var(--sp-focus)]"
                >
                  Sign out
                </button>
              </form>
