-- Employee-directory fence for CSV assignment imports (issue #328).
--
-- 20260806120000 widened the seat fence to the whole draft, but the import's
-- EMPLOYEE writes stayed unfenced: a CSV row naming a person overwrites that
-- matched public.employees row (full_name/position/department, active=true)
-- with no updated_at check, so a rename or deactivation committed by another
-- admin while the import review sat open was silently overwritten. The review
-- dialog shows exactly what the import will apply, so the employee directory
-- is reviewed state — the same argument that added the employee fence to
-- publish_seat_map in 20260806121000.
--
-- This migration re-creates the function with a trailing `expected_employees
-- jsonb default null` parameter and applies 20260806121000's active-directory
-- exact-match fence after the seat fence: active employee rows are locked in
-- the shared order (seats before employees, rows by id), then every active
-- row must match the (id, updated_at) the client rendered at CSV parse time
-- — same ids, same updated_at, no extras either way — raising SQLSTATE
-- 'MLS02' on any divergence. The client (DataUtilitiesPanel) captures the
-- active directory via lib/draftConcurrency listActiveEmployeeExpectations
-- alongside the existing seat expectations. Deliberately unchanged CSV
-- semantics (issue #328 acceptance criteria): blank position/department
-- still clear stored values, and matched imported employees are activated.
--
-- default null = fence skipped, so already-deployed application code calling
-- with (assignment_rows, expected_seats) keeps working during rollout. As
-- with the prior fence migrations the protection is one-directional: NEW
-- application code naming expected_employees cannot resolve against the OLD
-- signature, so this migration must land in the same merge as the client
-- change (it does — the Supabase GitHub integration applies migrations on
-- merge to main). The old signatures are dropped before recreating and
-- re-granting, or PostgREST calls would turn ambiguous; the 1-arg drop is
-- repeated defensively so the single-signature invariant survives replays.

drop function if exists public.import_assignments_csv(jsonb);
drop function if exists public.import_assignments_csv(jsonb, jsonb);
...
begin
  ...
  -- The import's employee-directory writes (overwriting matched rows,
  -- reactivating them, inserting new people) are reviewed state too: the
  -- review dialog shows exactly what will be applied. Lock the ACTIVE
  -- directory in the shared order (seats before employees, rows by id — the
  -- same order publish_seat_map uses) before checking its fence.
  --
  -- Known residual, shared with the seat fences (see 20260708120000): row
  -- locks cannot block a concurrent INSERT — or a reactivation of an
  -- inactive row, which these active-only locks never held — that commits
  -- between this fence statement and the writes below (READ COMMITTED gives
  -- each statement a fresh snapshot). Accepted here for the same reason it
  -- is accepted there.
  perform employee.id
  from public.employees as employee
  ...
  -- Employee-directory fence (mirrors 20260806121000): verify the ACTIVE
  -- directory still matches the (id, updated_at) the client rendered when
  -- the CSV was parsed for review. An edited or newly activated/created row
  -- fails the per-row match; a row deactivated since the review leaves the
  -- client holding more expectations than the directory has active rows,
  -- which the count check catches. Inactive rows are not fenced — the client
  -- only ever holds the active directory (the same active-only scope as
  -- 20260806121000), so a concurrent edit to an inactive row that a CSV name
  -- happens to match stays outside the fence.
...
revoke all on function public.import_assignments_csv(jsonb, jsonb, jsonb) from public, anon, authenticated;
grant execute on function public.import_assignments_csv(jsonb, jsonb, jsonb) to authenticated;
