-- Phase 2: let the inspector atomically MOVE an already-seated employee.
--
-- Previously the conflict branch always raised "That employee is already assigned
-- to %." and the action re-threw it, so in production Next.js stripped the message
-- to a generic digest. Phase 1 made the action RETURN the conflict; this migration
-- completes the fix on the database side:
--
--   * The conflict now carries a stable custom SQLSTATE 'MLS01' and the occupied
--     seat label in DETAIL, so the action can classify it without string-parsing.
--   * A new trailing `force_move boolean default false` parameter, when true,
--     vacates the employee's other draft seat in the SAME transaction instead of
--     raising. The whole function is one transaction (mirroring
--     swap_draft_seat_assignments), so the vacate + assign is atomic and safe.
--
-- The new parameter changes the function signature, which would otherwise leave
-- the 12-argument overload callable, so drop it before recreating and re-grant.
...
revoke all on function public.update_draft_seat(uuid, text, public.seat_status, uuid, text, text, boolean, text, boolean, text, text, text, boolean) from public, anon, authenticated;
grant execute on function public.update_draft_seat(uuid, text, public.seat_status, uuid, text, text, boolean, text, boolean, text, text, text, boolean) to authenticated;
