-- restore_draft_snapshot never staged label/seat_key writes the way its
-- sibling reset_draft_seats_to_published now does
-- (20260724150000_reset_draft_staged_writes.sql). seats_unique_label_per_layer
-- and seats_unique_key_per_layer are both non-deferrable unique indexes, and
-- the restore loop's UPDATE statements run one row at a time in id order — so
-- a snapshot that PERMUTES two draft seats' labels (or seat_keys) relative to
-- the live draft (e.g. an admin swapped two labels, then hit Undo) makes the
-- first row's UPDATE collide with a not-yet-updated row later in the loop and
-- the whole RPC aborts with a raw 23505 duplicate-key error. In production
-- that surfaces as a digest-stripped error and the undo/restore is lost.
--
-- Fix: park both label AND seat_key on a collision-free temporary value for
-- every surviving draft row the snapshot will change, before the loop runs.
-- Both columns are parked together because a snapshot can permute either (or
-- both) independently, and parking only one still risks colliding on the
-- other index. Unlike reset's staging step (which joins on
-- coalesce(seat_key, label), since reset matches published rows to draft rows
-- by that key), the restore loop matches rows by `id`, so the parking join is
-- by `id` too.
--
-- Everything else about the RPC is unchanged from
-- 20260708120000_draft_concurrency_fence.sql — only the new staging statement
-- is inserted between the custom-seat delete block and the restore loop.
