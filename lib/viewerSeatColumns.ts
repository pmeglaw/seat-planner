...
//
// `floor` (20260901120000) joined the list in multi-floor PR-2, its first
// consumer: every viewer surface dispatches on it (which plan, which roster,
// which floor a find lands on).
export const VIEWER_SEAT_COLUMNS =
  "id,seat_key,label,x,y,status,layer,employee_id,zone,department,is_custom,floor,created_at,updated_at";
...
// What a viewer seat query actually returns — Seat minus the one column the
