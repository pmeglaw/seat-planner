...
// general in-app navigation (the shell's <Link> transitions stay soft):
//
// 1. Post-auth redirects (LoginForm, UpdatePasswordForm).
// 2. Deploy-skew fallback (components/ui/useShellNavigation.ts via
//    lib/deploySkew.ts): a stale tab's soft navigation would fetch RSC from a
//    newer deployment and dead-end into the router's own delayed full-reload
//    fallback, so the shell takes the full load deliberately on the first
//    click instead.
// 3. The shell's stalled-navigation watchdog (useShellNavigation, #316): a
//    soft nav whose pathname hasn't committed after 4s is restarted as a full
//    document load — the same stall class as the auth racing pair below.
//
...
//   the shell's nav watchdog backstops (#316) — parking the user on the
