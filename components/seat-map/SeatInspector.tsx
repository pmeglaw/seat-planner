...
function InspectorSection({
  ...
  return (
    <details open={defaultOpen} className="group border-b border-white/10">
      <summary
        className="flex cursor-pointer select-none list-none items-center px-4 py-2.5 text-[12px] font-medium text-[#e4e4e4] transition hover:bg-[#1f1f1f] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[var(--admin-primary)] [&::-webkit-details-marker]:hidden"
      >
        <span id={headingId}>{title}</span>
        <svg aria-hidden="true" viewBox="0 0 20 20" className="ml-auto h-3.5 w-3.5 text-[var(--admin-chrome-muted)] transition-transform duration-150 group-open:rotate-90">
          <path d="m8 5.5 4.5 4.5L8 14.5" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      </summary>
...
export function SeatInspector({
  ...
  return (
    ...
    <aside
      ...
      className="fixed inset-x-3 bottom-3 z-[80] flex max-h-[60vh] flex-col overflow-hidden border border-white/10 bg-[var(--admin-chrome-bg)] text-[#f4f4f4] shadow-[var(--admin-elevation-4-shadow)] motion-safe:animate-[sp-panel-in_220ms_cubic-bezier(0.2,0,0,1)] panel:inset-x-auto panel:bottom-0 panel:right-0 panel:top-10 panel:z-40 panel:max-h-none panel:w-[320px] panel:max-w-[calc(100vw-1.5rem)] panel:border-0 panel:border-l panel:border-white/10 panel:shadow-none"
    >
      ...
      {canEdit ? (
        <form id="seat-inspector-form" onSubmit={handleSubmit} noValidate className="flex min-h-0 flex-1 flex-col">
          <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
            ...
            {editingAssignment && (
            <section aria-labelledby="seat-assignment-heading" className="border-b border-white/10 px-4 py-3">
            ...
            <InspectorSection title="Seat" headingId="seat-details-heading">
              <dl>
            ...
            <InspectorSection title={hasAssignedPerson ? <>Notes</> : <>Status &amp; notes</>} headingId="seat-metadata-heading">
              {!hasAssignedPerson && (
                <label className="block">
                  <span className="text-[11px] font-medium tracking-normal text-[var(--admin-chrome-muted)]">Seat status</span>
                  <select
                    ref={statusRef}
              ...
              <label className="mt-3 block">
                <span className="text-[11px] font-medium tracking-normal text-[var(--admin-chrome-muted)]">Notes</span>
                <textarea
                  ref={notesRef}
                  value={form.notes}
          ...
          <div className="sticky bottom-0 z-20 border-t border-white/10 bg-[var(--admin-chrome-bg)] px-4 py-3">
            ...
            {/* Draft-impact pill (announced via the sr-only live region above). */}
            <div aria-hidden="true" className={["mt-2 flex items-center gap-2 px-3 py-2 text-xs font-medium", inspectorStatePillClassName].join(" ")}>
