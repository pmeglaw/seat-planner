...
test("seat badges use compact map-native labels with strong active states", async () => {
  ...
  assert.match(markerSource, /border-\[#AEB4BA\]\/95 bg-\[#F2F2F3\]\/95/);
...
test("selected inspector and search results stay attached to the map workspace", async () => {
  ...
  assert.match(inspectorSource, /panel:max-h-none[\s\S]*panel:w-\[320px\][\s\S]*panel:rounded-\[14px\]/);
  ...
  assert.match(inspectorSource, /sticky top-0 z-20[\s\S]*bg-\[var\(--sp-color-surface\)\][\s\S]*id="seat-inspector-title"/);
