...
test("snapshot restore: a valid snapshot opens the review, and confirming sends the snapshot plus the seat fence", async () => {
  ...
  // UX-4 (audit 2026-09-10): a whole-draft replace confirms at danger weight,
  // matching the management deactivate confirm — never the sheet's primary.
  assert.ok(screen.getByRole("button", { name: "Restore draft snapshot" }).className.includes("cds-btn--danger"), "the restore confirm is danger weight");
