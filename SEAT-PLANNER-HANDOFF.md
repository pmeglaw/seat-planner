...
2. **There is no staging database.** Local dev points at live Supabase. **Two machines now hold live credentials.** The publish guard shipped fail-closed in #443 (v1.58.0); draft edits stay deliberately unguarded — and live passes have written prod twice (#476 employee no-op save). **Merging to `main` also auto-applies `supabase/migrations/` to that same live database — every schema PR writes production on merge, and no guard equivalent to the publish guard exists on that path.**
...
| **#476 (PR-4)** | v1.67.0 | Error-behind-modal (F-INT-4 / F-FRM-1) — employee-save errors render inside the still-open dialog (focused `role="alert"`, dismiss returns focus to Save); swap's thrown path gets the publish-style inline error + one-channel banner suppression; guard `dialog-error-placement` (ct over 5 open-through-error dialogs + a registry classifying all 14: ct / closes-before-resolve (PR-5) / closes-after-resolve-by-design with the MLS02 stale-fence rationale) |
| **#477 (PR-5)** | v1.68.0 | Pending indication + SR busy on all **20** mutating flows (re-measure superseded the audit's 17) — Carbon inline loading via the Button `loading` prop (16px spinner + participle + disabled), 4 closes-before-resolve dialogs reopened to hold through resolve with PR-4-style inline errors, inspector move-conflict same, undo/redo spinner-replaces-glyph, per-surface always-mounted sr-only "Working…" regions, `/my-seat` + `/login` own `loading.tsx` sentences; inspector-guard Save ledgered `closes-into-announcing-surface` (owner ruling 2026-08-27); guard `pending-state-source` (20-flow registry) + ct over the reopened dialogs. **Last PR of the audit arc — PR-6…11 frozen pending the redesign (`docs/redesign` branch).** |
...
**Pass-2 audit landed 2026-08-26:** `docs/design-system/AUDIT-2.md` — motion, spacing, the seventeen patterns, shell, keyboard, dark parity, and absences; ranked findings in its §0, guard map in §9 (spacing findings corrected same day against the fixed Carbon scale, skill v1.2.0; the docs-site-unreliable rule added in v1.2.1). **PR-1 shipped same day (#471, v1.64.0):** digest-leak error paths, silent vacate, double-click single-flight, five first-run states — §0 items 1 and 3 closed, item 6 partial. **PR-2 shipped (#474, v1.65.0):** 44px touch-target hit expansion off the map canvas under the no-overlap rule (54 of 58 specs; capped sites ledgered in `tests/touch-target-source.test.mjs`). **PR-3 shipped (#475, v1.66.0):** `/auth/update-password` dark theme (F-DK-1) — the auth scope is now guarded token-only by `tests/auth-theme-source.test.mjs`. **PR-4 shipped (#476, v1.67.0):** error-behind-modal closed — 2 of 14 dialog submits fixed (employee form, swap thrown path); the `dialog-error-placement` registry classifies all 14. **PR-5 shipped (#477, v1.68.0):** pending indication + SR busy on all 20 mutating flows — §0 item 6 fully closed; the closes-before-resolve dialogs reopened, the inspector-guard Save arm ledgered `closes-into-announcing-surface` (owner ruling 2026-08-27), guard `pending-state-source`. **The audit arc is paused at v1.68.0 — PR-6 through PR-11 are frozen, superseded pending the redesign, which starts from the `docs/redesign` branch** (shell-reference prototype committed there, never merged to main). The rail-over-employee-dialog overlap at 390px (#476 live pass, recorded not fixed) goes to the redesign's input list, not PR-11. **The staged plan — ruling queue, PR sequence, status board — is `docs/design-system/PLAN.md`; its diagram updates in the same commit as every PR that lands.**
