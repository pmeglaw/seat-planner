...
- Prod backups: `SUPABASE_DB_URL=… npm run backup:prod` (optional `SEAT_PLANNER_BACKUP_DIR`). The org is on the Free plan — **this script is the backup strategy**, not a convenience. It deliberately ignores `.env.local` (the URL must come from the process environment) and refuses to write a dump inside the repo; don't "fix" either property (`tests/backup-script-safety.test.mjs`).
...
`app/concepts/` holds **prototype-only** design surfaces, none of them part of the shipped viewer/admin flows, gated by `SEAT_PLANNER_ENABLE_PROTOTYPES` (a **build**-time flag) plus per-page `noindex`. The route list and the gate/status/noindex contract live in `app/concepts/CLAUDE.md`, which loads when you work under that directory.
